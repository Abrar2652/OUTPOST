...
def train(split_info, labels, graph, args, anomaly_info, logger, ppr_matrix=None):
    ...
    unlabeled_dataloader = NeighborSampler(graph.edge_index,
                                      ...
                                      num_workers=0,
                                      pin_memory=True)
    # built once, reused every epoch - see the note in eval()
    eval_dataloader = NeighborSampler(graph.edge_index,
                                      node_idx=None,
                                      sizes=args.sampling_sizes,
                                      batch_size=args.batch_size * int(args.get('eval_batch_mult') or 4),
                                      shuffle=False,
                                      drop_last=False,
                                      pin_memory=True)

    # same placement rationale as in train_outpost_v4: both arms of the
    # comparison must pay the same data-movement cost, or the efficiency table
    # measures where the features live rather than what the methods do
    from utils import features_to_device
    graph = features_to_device(graph, device,
                               enabled=args.get('features_on_gpu'))
    ...
    # val-selected / final bookkeeping, so the DEMO baseline reports the same two
    # selection protocols as OUTPOST. Observation only: nothing below feeds back
    # into the loss, the optimiser, or the epoch loop.
    best_val_auc = -1.0
    val_sel = {'auroc_all': 0.0, 'aupr_all': 0.0, 'auroc_unknown': 0.0,
               'aupr_unknown': 0.0, 'val_auc': 0.0}
    final = {'auroc_all': 0.0, 'aupr_all': 0.0, 'auroc_unknown': 0.0,
             'aupr_unknown': 0.0}
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.used_unlabeled_data:
            ...
            for i, (batch_size_unlabeled, n_id_unlabeled, adjs_unlabeled) in enumerate(unlabeled_dataloader):
                ...
                if args.used_augment_for_anormaly:
                    ...
                    # The weak view carries no gradient. In `consistency_loss`
                    # it produces only the hard pseudo-label (`max_idx`, an
                    # integer comparison) and the confidence mask (`.ge()` /
                    # `.le()`), and the loss itself is taken on logits_strong.
                    # Building its autograd graph is therefore pure cost - and
                    # not a small one: it doubles the activations retained over
                    # the whole unlabelled pool, which is what put DEMO over 24 GB
                    # on cs and made the paired cs comparison impossible to run.
                    # Values and gradients are unchanged. `demo_weak_nograd` is
                    # kept as a flag rather than hard-coded so the claim is
                    # testable: analysis/scripts/check_invariance.py --method
                    # demo runs it both ways and requires identical metrics.
                    if bool(args.get('demo_weak_nograd', True)):
                        with torch.no_grad():
        ...
        if epoch % 1 == 0:
            ...
            auroc_test_all_best, aupr_test_all_best, auroc_test_unknown_best, aupr_test_unknown_best, embedds_best, cur = (
                eval(model, split_info, rest_labels, graph, args, device, auroc_test_all_best, aupr_test_all_best, auroc_test_unknown_best, aupr_test_unknown_best, embedds_best, eval_dataloader))
            # val-selected bookkeeping: read-only, does not touch the update rule
            if cur['val_auc'] > best_val_auc:
                best_val_auc = cur['val_auc']
                val_sel = dict(cur)
            final = {k: cur[k] for k in ('auroc_all', 'aupr_all',
                                         'auroc_unknown', 'aupr_unknown')}
    ...
    best = {'auroc_all': auroc_test_all_best, 'aupr_all': aupr_test_all_best,
            'auroc_unknown': auroc_test_unknown_best,
            'aupr_unknown': aupr_test_unknown_best}
    return {'best': best, 'val_selected': val_sel, 'final': final,
            'n_params': sum(p.numel() for p in model.parameters())}


def eval(model, split_info, labels, graph, args, device, auroc_test_all_best, aupr_test_all_best, auroc_test_unknown_best, aupr_test_unknown_best, embedds_best, test_dataloader=None):
    model.eval()
    ...
    # Built once by the caller and reused, exactly as the OUTPOST arm does. The
    # sampler's CSR is deterministic and epoch-independent - shuffle=False, and
    # all the stochasticity lives in __iter__ - so reuse draws from the same
    # distribution while rebuilding it 400 times does not.
    #
    # This is a fairness fix, not an optimisation. Measured on Yelp, DEMO ran at
    # 62 s/epoch against OUTPOST's 7.6 s while both were doing the same work,
    # because only one of them cached its evaluation loader. Left alone, the
    # efficiency table would have reported an 8x speed advantage for OUTPOST
    # that belongs to the baseline's port rather than to either method, and the
    # 5-seed DEMO arm on Yelp would have cost 34 GPU-hours instead of 4.
    ...
    val_auc = (roc_auc_score(y_val, y_pred[idx_val])
               if len(np.unique(y_val)) > 1 else 0.5)
    cur = {'auroc_all': auroc_test_all, 'aupr_all': aupr_test_all,
           'auroc_unknown': auroc_test_unknown, 'aupr_unknown': aupr_test_unknown,
           'val_auc': float(val_auc)}
    return auroc_test_all_best, aupr_test_all_best, auroc_test_unknown_best, aupr_test_unknown_best, embedds_best, cur
