...
class NeighborSamplerShim:
    ...
    def __init__(self, edge_index, node_idx=None, sizes=(25, 10), batch_size=512,
        ...
        # This sampler is an index generator: nbr/ptr/deg are CPU tensors and the
        # caller moves the gathered features to the device. DEMO's dataloader can
        # hand us a CUDA node_idx, which would then index CPU tensors and raise
        # "indices should be either on cpu or on the same device". Pin to CPU.
        self.node_idx = node_idx.long().cpu()
...
def ad_split_num(labels, args, class_info):
    ...
    # binary datasets (yelp/tfinance) have no unseen anomaly classes
...
def ppr_power_iteration(graph, alpha=0.2, n_iter=20, self_loop=True,
                        dtype=np.float32):
    """PPR by power iteration: Z <- (1-a) A~ Z + a I, from Z = I.

    Same fixed point as the closed form a(I-(1-a)A~)^-1 used below, and the same
    thing dgl's APPNPConv(20, 0.2) computes, but without forming the inverse -
    peak memory is 2 dense N x N instead of ~4. That is what makes yelp
    (N=45,954) reachable at all. Verified against the closed form on photo.

    Still O(N^2) in the output, so ogbn-arxiv (229 GB) and ogbn-mag (4.3 TB)
    remain out of reach by any route.
    """
    n = graph.num_nodes
    src, dst = graph.edge_index.cpu().numpy()
    a = sp.coo_matrix((np.ones(len(src), dtype=dtype), (src, dst)),
                      shape=(n, n)).tocsr()
    a = a.maximum(a.T)                                   # undirected
    if self_loop:
        a = a + sp.eye(n, dtype=dtype, format="csr")
    d = np.asarray(a.sum(1)).ravel()
    dinv = np.power(np.maximum(d, 1e-12), -0.5).astype(dtype)
    at = sp.diags(dinv) @ a @ sp.diags(dinv)             # D^-1/2 A^ D^-1/2

    z = np.eye(n, dtype=dtype)
    eye_scaled = alpha * np.eye(n, dtype=dtype)
    for _ in range(n_iter):
        z = (1 - alpha) * (at @ z) + eye_scaled
    return z


def compute_ppr(graph, dataname: str, alpha=0.2, self_loop=True):
    # imported here rather than at module scope: the dgl branch below is
    # optional, and these three were lost when this file was consolidated.
    import networkx as nx
    from scipy.linalg import fractional_matrix_power, inv
    from torch_geometric.utils import to_networkx

    # The authors' released code covers photo/computers only (their README notes
    # the other datasets are still being prepared) and routes everything else
    # through dgl. photo/computers keep their exact closed form. cs and yelp take
    # the power-iteration route to the identical fixed point: the closed form
    # needs ~4 dense N x N copies (10.8 GB for cs, 68 GB for yelp), the iteration
    # needs 2 in float32 (1.3 GB and 17 GB).
    if dataname in ('cs', 'yelp'):
        print(f"[ppr] {dataname}: power iteration "
              f"(closed form needs ~{graph.num_nodes ** 2 * 8 * 4 / 1e9:.0f} GB)")
        return ppr_power_iteration(graph, alpha=alpha, self_loop=self_loop)
    if dataname in ('ogbn-arxiv', 'ogbn-mag'):
        raise MemoryError(
            f"PPR for {dataname} needs a dense {graph.num_nodes}^2 matrix "
            f"({graph.num_nodes ** 2 * 4 / 1e9:.0f} GB as float32). DEMO's mixup "
            f"cannot run at this scale by any route; run the baseline without it "
            f"(the paper's 'w/o Mix' variant) and report the limitation.")

    if dataname in ['photo', 'computers', 'cs']:
