"""Loss functions for the DEMO baseline.

DEMO's three components live here: DeviationLoss, the FixMatch-style
consistency_loss, the energy-gradient reweighting (compute_beta) and the
PPR-based anomaly_mixup. OUTPOST v4 uses plain BCE plus the terms defined
inline in trainer.py, so nothing here is on the OUTPOST path.

NOTE: NeighborSampler is aliased to our pure-torch shim (utils.get_neighbor_sampler)
because torch-sparse has no wheel for recent torch builds.
"""
